import { updateSlaveControlValidation } from 'src/app/user/health-history/health-history-validation-helper';
...
@Component({
  selector: 'app-enrollment-health-history',
  templateUrl: './enrollment-health-history.component.html',
  styleUrls: ['./enrollment-health-history.component.scss']
})
export class EnrollmentHealthHistoryComponent implements OnInit {
  public selectedVal = 5;
  stepCount: number = 0;
  @Input()
  set selectedCurrentIndex(value: number) {
    ...
    if (this.selectedVal == 5) {
      let allData = this.userService.getEnrolledItem();
      this.initHealthForm();
      this.childId = allData.child.id;
      this.healthHistoryForm?.controls?.['selectedChild'].patchValue(this.childId)
      this.updateData();
  ...
  async SubmitData() {
    ...
    if (this.healthHistoryForm.invalid) {
      this.notification.showError('Please clear the form errors.');
      return;
    }
    if (
  ...
  updateData() {
    this.userService.getChild(this.healthHistoryForm).subscribe({
      next: (data) => {
        let index = 0;
        data.data.forEach((el:any,i:any) => {
          if (el.id == this.childId) {
            this.illessSelected = el.illness;
            index = i;
          }
        });
  ...
  postHealth() {
    ...
    this.userService.postHealthHistory(dataToSend).subscribe({
      next: (data) => { },
    });
  }
