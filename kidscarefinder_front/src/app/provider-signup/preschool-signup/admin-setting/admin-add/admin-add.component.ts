import { Component, OnInit, ViewChild, TemplateRef, Output, EventEmitter, ElementRef, ViewChildren } from '@angular/core';
...
export class AdminAddComponent implements OnInit {
  ...
  @ViewChild('primaryAdminRef', { static: false }) public primaryAdminRef!: ElementRef
  primaryAdmin: string = '';
  ...
  initAdminEditForm() {
    this.editAdminForm = this._formBuilder.group({
      full_name: ['', [Validators.required, Validators.pattern(/^([a-z']+(-| )?)+$/i)]],
      last_name: ['', [Validators.required, Validators.pattern(/^([a-z']+(-| )?)+$/i)]],
      position: ['', Validators.required],
      email: ['', [Validators.required, Validators.pattern(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/s)]],
    ...
    this.providerService.getAllProviders().subscribe({
      next: (data) => {
        if (data?.code && data?.code == 200) {
          ...
          let admin = this.administratorList.find((x: any) => x.make_primary_admin == true);
        ...
        if (error?.error?.errors?.email) {
          this.notificationService.showError(CustomAlertMessage.adminSetting[0].message)

        }
        if (error?.error?.errors?.mobile) {
          this.notificationService.showError(CustomAlertMessage.adminSetting[1].message)
  ...
  selectDeselectAll() {
    ...
    for (const name in controls) {
      if (name !== 'first_name' && name !== 'last_name' && name !== 'position' && name !== 'email' && name != 'mobile' && name != 'make_primary_admin') {
  ...
  selectDeselectAllEdit() {
    ...
    this.editAdminForm.patchValue({ notifications: this.patchedData(this.editSelectAllNotification) })
  }
  ...
  saveData() {
    ...
    //sendData.mobile = sendData.mobile.replaceAll("-", "");
  ...
  openDialog() {
    const rect: DOMRect = this.primaryAdminRef.nativeElement.getBoundingClientRect()
  ...
  openCloseDiv(id: number) {
    ...
    if((<HTMLInputElement>document.getElementById(arroId)).className=="rotate90"){
      (<HTMLInputElement>document.getElementById(arroId)).classList.remove("rotate90");
    }else{
      (<HTMLInputElement>document.getElementById(arroId)).classList.add("rotate90");
    }
  ...
  editAdmin(id: number) {
    ...
    let dataToSend = {
      ...
      notification: {
        ...
        waitlist_notifications: this.editAdminForm?.value.notifications.waitlist_notifications,
        new_online_applications: this.editAdminForm?.value.notifications.new_online_applications,
        enrollment_requests: this.editAdminForm?.value.notifications.enrollment_requests,
        new_enrollment_packages: this.editAdminForm?.value.notifications.new_enrollment_packages,
        monthly_usage_summary: this.editAdminForm?.value.notifications.monthly_usage_summary,
        affiliated_provider_requests: this.editAdminForm?.value.notifications.affiliated_provider_requests,
        marketing_from_KCF_affiliates: this.editAdminForm?.value.notifications.marketing_from_KCF_affiliates
    ...
    this.providerService.updateProviderData(dataToSend, id).subscribe({
      next: (data) => {
        if (data?.code && data?.code == 200) {
          this.clickedId = 0;
          let updatedIndex = this.administratorList.findIndex((x: any) => x.id == id);
          if (updatedIndex > -1) {
            this.updatedIndexValues(this.administratorList, updatedIndex, dataToSend);
          }
        }
      }, error: (error: any) => {
        if (error?.error?.errors?.email) {
        ...
        if (data?.code && data?.code == 200) {
          if (foundIndex > -1) {
            ...
            this.editAdminForm.patchValue({
              ...
              mobile: this.phoneNumberFormatPipe.transform(adminstrator?.mobile),
  ...
  getKeyByValue(object: any) {
    ...
    if (keysArr.length) {
      ...
      let filteredWishlist: any = this.permissionNotifs.filter((x) => slugArr.has(x.slug))
