...
export class AccountTypeComponent implements OnInit {
  ...
  goToMyProviderPage(){
    ...
    this.router.navigateByUrl(`/preschool/${this.addressDetails.state?.replaceAll(' ','')}/${this.addressDetails.city?.replaceAll(' ','')}/${this.organization_name?.replaceAll(' ','')}/${this.providerId}`);
  }
...
(async () => {
    ...
    this.providerService.getProviderAddressDetails().subscribe({
      next: (data: any) => {
        if (data && data?.data && data?.code == 200) {
          ...
          this.providerUrl = `${window.origin.split('//')[1]}/preschool/${this.addressDetails.state?.replaceAll(' ','')}/${this.addressDetails.city?.replaceAll(' ','')}/${this.organization_name?.replaceAll(' ','')}/${this.providerId}`
        }
      },
    });
  }
  ...
  basicPlanSelect(){
    this.providerService.updateProviderSignupSteps({"step10_completed" : true},this.localStrService.getUser()?.provider_id)
    .subscribe({
      next: (data)=>{
  ...
  savePlanInfo(planId: any, monthly?: boolean){
    ...
    this.subscriptionPlans.forEach((element: any) => {
      ...
      if(monthly){
        this.monthly = true;
        this.amount = String(this.defaultPlan?.monthly_subscription_price) + '00'
      }
      else{
        this.monthly = false;
  ...
  userDetails(){
    if(this.localStrService.getUser()?.id!=undefined){
      this.userService.getUserById(this.localStrService.getUser()?.id).subscribe({
        next: (res: any) => {
          ...
          this.affiliated_provider_url = this.user?.provider?.affiliated_provider_url;
          this.nextPaymentDate = this.user?.provider?.subscription_expiration_time
        },
        error: (error: any) => {
        },
      })
  ...
  checkFormValidity(stepCount: number, callback: any) {
		switch (stepCount) {
			case 0:     if (!this.selectedPlan) {
        this.notificationService.showWarning(CustomAlertMessage.paymentSection[0].message);
      ...
      if(this.type == 2){
        ...
        this.providerService.cardDetails(obj).subscribe({
          next: (res: any) => {
            // this.notificationService.showSuccess(res.data.seller_message);
            this.notificationService.showSuccess(CustomAlertMessage.paymentSection[3].message);
            let data = {
              provider_subscription: this.selectedPlan
            }
            this.providerService.saveProviderWebsiteInfo(data,this.providerId).subscribe({
              next: (data) => {
            ...
            .subscribe({
              next: (data)=>{
                ...
                // this.providerService.updateProviderSignupSteps({"affiliated_provider_url" : this.providerUrl, "unique_provider_id": this.uniqueId },this.localStrService.getUser()?.provider_id)
                // .subscribe((res)=>{
                //   return callback(true);
                // })
                return callback(true);
                // this.router.navigate(['/providerSettings/account-billing'])
              },
              error: (error)=>{
        ...
        obj.routing_number = Number(this.bankDetailForm.value?.routing_number);
        ...
        this.providerService.bankAccount(obj).subscribe({
          next: async (res: any) => {
            let data = res.data;
            this.notificationService.showSuccess(CustomAlertMessage.paymentSection[2].message)
            ...
            .then(({paymentIntent, error}: any) => {
              if (error) {
                ...
                this.notificationService.showError(`Customer canceled the hosted verification modal`);
                return callback(false);
                ...
                // the mandate.
                  ...
                  .then(({paymentIntent, error}) => {
                    if (error) {
                      ...
                      this.notificationService.showError('Confirmation failed');
                      return callback(false);
                      // Confirmation failed. Attempt again with a different payment method.
                    } else if (paymentIntent.status === "processing") {
                      this.notificationService.showSuccess('Confirmation succeeded! The account will be debited')
                      // this.router.navigateByUrl(`/providerSettings/user-activity`)
                      ...
                      this.notificationService.showError('The account needs to be verified via microdeposits')
                      return callback(false);
