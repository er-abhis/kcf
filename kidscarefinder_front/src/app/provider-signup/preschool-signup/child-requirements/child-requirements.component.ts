...
@Component({
  selector: 'provider-step-four-component',
  templateUrl: './child-requirements.component.html',
  styleUrls: ['./child-requirements.component.scss'],
})
export class ChildRequirementsComponent implements OnInit {
  stepCount: number = 0;
  selectedVal = 4;
  ...
  checkFormValidity(stepCount: number, callback: any) {
    switch (stepCount) {
      case 0:
        ...
        if(this.childNeedCareGroup.value.is_minimum_age == 'Yes' && (this.childNeedCareGroup.value.minimum_age_months == null || this.childNeedCareGroup.value.minimum_age_years == null ) ){
          this.notificationService.showWarning(CustomAlertMessage.childReq[1].message)
        ...
        return this.providerService
          .childRequiremnet(this.childNeedCareGroup.value)
          .subscribe({
            next: (data: any) => {
