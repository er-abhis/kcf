...
@Component({
  selector: 'app-in-home-daycare-schedule-availability',
  templateUrl: './in-home-daycare-schedule-availability.component.html',
  styleUrls: ['./in-home-daycare-schedule-availability.component.scss']
})
export class InHomeDaycareScheduleAvailabilityComponent implements OnInit {
  ...
  checkFormValidity(stepCount: number, callback: any) {
    switch (stepCount) {
      ...
      case 4: {
        ...
        return this.providerService
          .saveProviderAgeGroup(true, sendData)
          .subscribe({
            next: (data) => {
              if (data?.code && data?.code == 200) {
                callback(true);
