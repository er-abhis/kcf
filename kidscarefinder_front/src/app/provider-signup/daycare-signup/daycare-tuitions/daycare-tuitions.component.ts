...
@Component({
  selector: 'app-daycare-tuitions',
  templateUrl: './daycare-tuitions.component.html',
  styleUrls: ['./daycare-tuitions.component.scss'],
})
export class DaycareTuitionsComponent implements OnInit {
  selectedVal = 7;
  ...
  checkFormValidity(stepCount: number, callback: any) {
    switch (stepCount) {
      case 0:
        ...
        dataToSend.tuitionfees_time =
          this.childTutionGroup.value.tuitionfees_time || '';
        this.Submitted = true;
        if (
          this.childTutionGroup.value.tutionFee &&
          !this.childTutionGroup.value.tuitionfees_time
          ...
          this.notificationService.showError(
            CustomAlertMessage.tuition[1].message
          );
          return callback(false);
        }
          ...
          return this.providerService.photo(this.pdfData).subscribe({
            next: (data: any) => {
              if (data && data.data && data.code == 200) {
                this.childTutionGroup.controls.tutionPdf.patchValue(
                  data?.data?.upload_file
                );
                dataToSend.pdf_link = this.childTutionGroup.value.tutionPdf;
                this.providerService
                  .tuitions(dataToSend, this.update)
                  .subscribe({
                    next: (data: any) => {
                    ...
                    error: (error: any) => {
                      if (error?.error.data) {
                        if (error?.error.errors?.tuitionfees[0]) {
                          this.notificationService.showError(
                            'tuition fees: ' +
                              error?.error.errors?.tuitionfees[0]
                          );
                        }
                        if (error?.error.errors?.web_link[0]) {
                          this.notificationService.showError(
                            'web link: ' + error?.error.errors?.web_link[0]
                          );
                        }
                        if (error?.error.errors?.pdf_link[0]) {
                          this.notificationService.showError(
                            'pdf file: ' + error?.error.errors.pdf_link[0]
                          );
                        }
                      } else {
                        this.notificationService.showError(
            ...
            .subscribe({
              ...
              error: (error: any) => {
                if (error?.error.data) {
                  if (error?.error.errors?.tuitionfees[0]) {
                    this.notificationService.showError(
                      'tuition fees: ' + error?.error.errors?.tuitionfees[0]
                    );
                  }
                  if (error?.error.errors?.web_link[0]) {
                    this.notificationService.showError(
