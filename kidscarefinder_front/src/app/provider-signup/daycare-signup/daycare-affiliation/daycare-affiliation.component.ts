...
@Component({
  selector: 'app-daycare-affiliation',
  templateUrl: './daycare-affiliation.component.html',
  styleUrls: ['./daycare-affiliation.component.scss'],
  providers: [
    {
...
export class DaycareAffiliationComponent implements OnInit {
  stepCount: number = 0;
  selectedVal = 8;
  ...
  searchValueLanguage() {
    ...
    var check = this.toppingListlanguage.filter(
      (item: any) =>
        item.language.toLowerCase().indexOf(search.toLowerCase()) > -1
    );
    this.filteredToppingListlanguage.next(check);
    if (check.length == 0) this.languageMesg = true;
  }
  ...
  searchValueProvider() {
    ...
    var check = this.toppingListReligious.filter(
      (item: any) =>
        item.religion.toLowerCase().indexOf(search.toLowerCase()) > -1
    );
    this.filteredProvidersList.next(check);
    if (check.length == 0) this.religionMesg = true;
  }
  ...
  searchProvider() {
    this.providerService
      .searchAffiliatedprovider(this.affiliatedProviderData)
      .subscribe(
        (response: any) => {
          if (response.data.length && response.data[0].id) {
            ...
            if (!ids.includes(response.data[0].id)) {
              ...
              this.notificatioService.showError(
                CustomAlertMessage.affiliation[4].message
              );
          } else
            this.notificatioService.showError(
              CustomAlertMessage.affiliation[5].message
            );
        },
        (error) => {
          this.notificatioService.showError(
    ...
    this.providerService
      ...
      .subscribe({
        next: (res: any) => {
          ...
          this.addedAffiliationsId = this.affiliatedProvidersList.map(
            (item: any) => item.id
          );
          if (
            this.affiliatedProvidersList.length &&
            this.ChildNeedCare.value.affiliatedProvider == 'Yes'
          ) {
  ...
  deleteAffiliatedProvider(id?: any) {
    if (id) {
      this.providerService
        .delAffiliatedprovider(localStorage.getItem('provider_id'), id)
        .subscribe({
          next: (res: any) => {},
          error: (error: any) => {
  ...
  saveData() {
    if (this.affiliatedProviderData.trim()) this.searchProvider();
  }
  ...
  checkFormValidity(stepCount: number, callback: any) {
    switch (stepCount) {
      case 0:
        console.log(this.affliationGroup.value);
        if (!this.affliationGroup.value.religiousHave) {
          this.notificatioService.showError(
            CustomAlertMessage.affiliation[0].message
          );
        ...
        if (this.affliationGroup.value.religiousHave == 'Yes') {
          if (!this.affliationGroup.value.toppingsReligious?.length) {
            this.notificatioService.showWarning(
              CustomAlertMessage.affiliation[1].message
            );
            return callback(false);
          }
          dataToSend = {
      ...
      case 1:
        ...
        if (this.ChildNeedCareGroup.value.langHave == 'Yes') {
          if (!this.ChildNeedCareGroup.value.toppingslanguage?.length) {
            this.notificatioService.showWarning(
              CustomAlertMessage.affiliation[2].message
            );
            return callback(false);
          }
          dataToSendlang = {
      ...
      case 3:
        ...
        if (dataToSendProv.is_affiliated_provider) {
          if (!this.affiliatedProvidersList.length) {
            this.notificatioService.showError(
              CustomAlertMessage.affiliation[3].message
            );
            return callback(false);
          }
          let body: any = [];
          for (let i = 0; i < this.affiliatedProvidersList.length; i++) {
            if (
              !this.addedAffiliationsId.includes(
                this.affiliatedProvidersList[i].id
              )
            )
              body.push({
          ...
          return this.providerService.addAffiliatedprovider(body).subscribe({
            next: (data: any) => {
              return this.providerService
                .affiliation(dataToSendProv)
                .subscribe({
                  next: (data: any) => {
  ...
  search(event: any) {
    ...
    this.filteredProvidersList = this.toppingListReligious.filter((x: any) =>
      x.religion.toUpperCase().match(searchData.toUpperCase())
    );
  }

  searchLang(event: any) {
    let searchData = event.target.value;
    this.filteredToppingListlanguage = this.toppingListlanguage.filter(
      (x: any) => x.language.toUpperCase().match(searchData.toUpperCase())
    );
  }
}
