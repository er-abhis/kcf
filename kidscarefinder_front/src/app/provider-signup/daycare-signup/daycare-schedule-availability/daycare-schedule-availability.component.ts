...
@Component({
  selector: 'app-daycare-schedule-availability',
  templateUrl: './daycare-schedule-availability.component.html',
  styleUrls: ['./daycare-schedule-availability.component.scss'],
})
export class DaycareScheduleAvailabilityComponent implements OnInit {
  ...
  allowdropGroup = this._formBuilder.group({
    is_potty_trained: [''],
  });

  getProgramsOffered() {
    return this.programsOffers
      .filter((item: any) => item != this.INFANT)
      ?.join('/ ');
  }
  ...
  intializeObservedHolidayForm() {
    this.observedHolidayFormGroup = this._formBuilder.group({
      observed_holiday_web_link: [
        '',
        Validators.pattern(new RegExp(this.urlRegex, 'i')),
      ],
    });
  }
  ...
  availableNowChange(event: MatRadioChange) {
    if (event.value === 'Yes') {
      ...
      this.availabileNowFormGroup.controls[
        'availability_in_3_months'
      ].setValidators(null);
      this.availabileNowFormGroup.controls[
        'availability_in_3_months'
      ].updateValueAndValidity();
      this.availabileNowFormGroup.controls[
        'availability_in_6_months'
      ].setValidators(null);
      this.availabileNowFormGroup.controls[
        'availability_in_6_months'
      ].updateValueAndValidity();
      this.availabileNowFormGroup.controls[
        'availability_in_12_months'
      ].setValidators(null);
      this.availabileNowFormGroup.controls[
        'availability_in_12_months'
      ].updateValueAndValidity();
  ...
  radioChange(event: MatRadioChange, data: any) {
    if (data === 'Yes') this.beforeAfterCare = true;
    else this.beforeAfterCare = false;
  }

  checkFormValidity(stepCount: number, callback: any) {
    switch (stepCount) {
      case 0: {
        let { isValid, message, data } = this.checkValidityAndFormatData(
          this.scheduleAvailable.value.scheduleAvailability
        );
        if (!isValid) {
          this.notificationService.showError(
            CustomAlertMessage.startEndTime[0].message
          );
          return callback(false);
        }
        let schedule = <FormArray>(
          this.scheduleAvailable.controls['scheduleAvailability']
        );
        schedule.value.map((control: any) => {
          let keys = Object.keys(this.daysObj);
          keys.map((key: any) => {
            if (control[key] == true) this.daysObj[key]++;
          });
        });
        ...
        return this.providerService.addProviderSchedule(data).subscribe({
          ...
          error: (error: any) => {
            ...
            this.notificationService.showError(
              CustomAlertMessage.startEndTime[1].message
            );
            callback(false);
          },
        });
      ...
      case 1:
        ...
        if (this.beforeAfterCareGroup.value.is_before_after_care == 'Yes') {
          let { isValid, message, data } = this.checkValidityAndFormatData(
            this.beforeAfterCareGroup.value.scheduleAvailability
          );
          if (!isValid) {
            this.notificationService.showError(
              CustomAlertMessage.startEndTime[0].message
          ...
          let schedule = <FormArray>(
            this.beforeAfterCareGroup.controls['scheduleAvailability']
          );
          schedule.value.map((control: any) => {
            let keys = Object.keys(this.daysObj);
            keys.map((key: any) => {
              ...
              if (data?.code && data?.code == 200) {
                this.providerService
                  .saveProviderWebsiteInfo(
                    {
                      is_before_after_care:
                        this.beforeAfterCareGroup.value.is_before_after_care ==
                        'Yes'
                          ? true
                          : false,
                    },
        ...
        this.providerService
          .saveProviderWebsiteInfo(
            {
              is_before_after_care:
                this.beforeAfterCareGroup.value.is_before_after_care == 'Yes'
                  ? true
                  : false,
            },
            this.providerId
      ...
      case 2:
        ...
        if (this.spaceAvailableINPUT) {
          ...
          if (!spaceAvailable) {
            this.notificationService.showWarning(
              CustomAlertMessage.availability[1].message
            );
            return callback(false);
          }
        }
        if (!this.availabileNowFormGroup.valid) {
          this.notificationService.showWarning(
            CustomAlertMessage.availability[0].message
          );
        ...
        let sendata: any = {
          availability_now:
            this.availabileNowFormGroup?.value?.availability_now == 'Yes'
              ? true
              : false,
          availability_in_3_months:
            this.availabileNowFormGroup?.value?.availability_in_3_months ==
            'Yes'
              ? true
              : false,
          availability_in_6_months:
            this.availabileNowFormGroup?.value?.availability_in_6_months ==
            'Yes'
              ? true
              : false,
          availability_in_12_months:
            this.availabileNowFormGroup?.value?.availability_in_12_months ==
            'Yes'
              ? true
              : false,
        };
        ...
        return this.providerService
          .savedaycareProviderAgeGroup(
            true,
            this.availabileNowFormGroup.value.ageGroup
          )
          .subscribe({
            next: (data) => {
              if (data?.code && data?.code == 200) {
      ...
      case 3:
        if (!Object.values(this.timeFormGroup.value).includes(true)) {
          this.notificationService.showWarning(
            CustomAlertMessage.scheduleOffer[0].message
          );
          return callback(false);
        }
        ...
        break;
      case 4: {
        let { isValid, message, sendData } = this.formatNapTimeValue(
          this.napFormGroup.value
        );
        if (!isValid) {
          this.notificationService.showError(
            CustomAlertMessage.scheduleOffer[1].message
          );
        ...
        return this.providerService
          .savedaycareProviderAgeGroup(true, sendData)
          .subscribe({
            next: (data) => {
              if (data?.code && data?.code == 200) {
                callback(true);
          ...
          return this.providerService.photo(this.pdfData).subscribe({
            next: (data: any) => {
              if (data && data.data && data.code == 200) {
                ...
                if (
                  this.observedHolidayFormGroup.value?.observed_holiday_web_link
                ) {
                  obj.observed_holiday_web_link =
                    this.observedHolidayFormGroup.value?.observed_holiday_web_link;
                }
                if (this.fileUrl) {
                ...
                this.providerService
                  .saveProviderWebsiteInfo(obj, this.providerId)
                  .subscribe({
                    next: (x) => {
                      return callback(true);
                    },
      ...
      case 6:
        if (this.allowdropGroup.invalid) {
          this.notificationService.showError(
        ...
        return this.providerService
          .childRequiremnet(this.allowdropGroup.value)
          .subscribe({
            next: (data: any) => {
  ...
  addPreschoolAvailable(isbeforeAfter = false) {
    let schedule = !isbeforeAfter
      ? <FormArray>this.scheduleAvailable.controls['scheduleAvailability']
      : <FormArray>this.beforeAfterCareGroup.controls['scheduleAvailability'];
    this.disableAddBtn =
      schedule && schedule.controls.length > 4 ? true : false;
    schedule.push(
  ...
  get scheduleAForm() {
    return (<FormArray>this.scheduleAvailable.get('scheduleAvailability'))
      .controls;
  }

  get scheduleAfterBeforeForm() {
    return (<FormArray>this.beforeAfterCareGroup.get('scheduleAvailability'))
      .controls;
  }
  ...
  checkValidityAndFormatData(data: any) {
    ...
    if (newData.length) {
      for (let i = 0; i < newData.length; i++) {
        ...
        if (
          newData[i].monday == false &&
          newData[i].tuesday == false &&
          newData[i].wednesday == false &&
          newData[i].thrusday == false &&
          newData[i].friday == false &&
          newData[i].saturday == false &&
          newData[i].sunday == false
        ) {
          return {
            isValid: false,
        ...
        newObj.start_time = this.convertStartAndEndTime(
          newData[i].startHour,
          newData[i].startMinute,
          newData[i].startFormat
        );
        newObj.end_time = this.convertStartAndEndTime(
          newData[i].endHour,
          newData[i].endMinute,
          newData[i].endFormat
        );
    ...
    switch (stepNumber) {
      ...
      case 1:
          ...
          next: (data) => {
            if (data?.data?.is_before_after_care != null)
              this.beforeAfterCareGroup.patchValue({
                is_before_after_care: data?.data?.is_before_after_care
                  ? 'Yes'
                  : 'No',
              });
        ...
        this.providerService.getProviderSchedule().subscribe((data: any) => {
          if (data && data?.data && data?.code == 200) {
            let beforeAfter = data.data.filter(
              (x: any) => x.schedule_type != 'Regular'
            );
      ...
      case 2:
        ...
        let availGrp = <FormArray>(
          this.availabileNowFormGroup.controls['ageGroup']
        );
        availGrp.clear();
        this.providerService.getdaycareProviderAgeGroups().subscribe({
          next: (data) => {
            if (data && data?.data && data?.code == 200) {
      ...
      next: (data) => {
        if (data && data?.data && data?.code == 200) {
          ...
          this.napFormGroup.patchValue(
            this.patchNapData(infantData, toddlerData)
          );
        }
      },
      error: (error: any) => {},
      ...
      next: (data) => {
        if (step == 2) {
          ...
          if (data?.data?.availability_in_3_months != null) {
            this.availabileNowFormGroup.patchValue({
              availability_in_3_months: this.setYesNo(
                data?.data?.availability_in_3_months
              ),
            });
          }
          if (data?.data?.availability_in_12_months != null) {
            this.availabileNowFormGroup.patchValue({
              availability_in_12_months: this.setYesNo(
                data?.data?.availability_in_12_months
              ),
            });
          }
          if (data?.data?.availability_in_6_months != null) {
            this.availabileNowFormGroup.patchValue({
              availability_in_6_months: this.setYesNo(
                data?.data?.availability_in_6_months
              ),
            });
          }
          ...
          if (data?.data?.availability_now) {
            ...
            if (data?.data?.availability_in_3_months) {
              ...
              data?.data?.availability_now != null &&
          ...
          this.observedHolidayFormGroup.patchValue({
            observed_holiday_web_link: data?.data?.observed_holiday_web_link,
          });
        }
      },
      error: (error: any) => {
  ...
  addFormControlsWithData(data: any, beforeAfter = false) {
    ...
    data.forEach((scheduleData: any, index: number) => {
      ...
      schedule.push(
        this._formBuilder.group({
          startHour: this.providerService.twelveHourFormatFromTwentyFour(
            scheduleData.start_time,
            'hour'
          ),
          startMinute: this.providerService.twelveHourFormatFromTwentyFour(
            scheduleData.start_time,
            'minutes'
          ),
          startFormat: this.providerService.twelveHourFormatFromTwentyFour(
            scheduleData.start_time,
            'format'
          ),
          endHour: this.providerService.twelveHourFormatFromTwentyFour(
            scheduleData.end_time,
            'hour'
          ),
          endMinute: this.providerService.twelveHourFormatFromTwentyFour(
            scheduleData.end_time,
            'minutes'
          ),
          endFormat: this.providerService.twelveHourFormatFromTwentyFour(
            scheduleData.end_time,
            'format'
          ),
    ...
    if (ev.checked) {
      if (ev.source.value) {
        ...
        if (newData && newData.length) {
          let fndIndex = schedule.value.findIndex(
            (x: any, index: number) => x[str] == true && index !== nindex
          );
          if (fndIndex > -1) {
  ...
  removeAfterSchedule(index: number) {
    let formGrp = <FormArray>(
      this.beforeAfterCareGroup.get('scheduleAvailability')
    );
    formGrp.removeAt(index);
  }
  ...
  formatNapTimeValue(data: any) {
    if (!data?.not_applicable) {
      if (data?.is_infant) {
        if (
          data?.morning_start_hour == null ||
          data?.morning_end_hour == null ||
          data?.afternoon_end_hour == null ||
          data?.afternoon_start_hour == null
        ) {
          return {
            isValid: false,
    ...
    if (
      this.programsOffers.includes(this.TODDLER) ||
      this.programsOffers.includes(this.PREK) ||
      this.programsOffers.includes(this.PRESCHOOL)
    )
  ...
  getArrayByKey(key: string, data: any, onlyMorning: boolean = false) {
    ...
    let resp: any = newCategories.find(
      (x: any) => x?.agegroupcategory?.agegroupcategory == key
    );
    if (resp && !this.isEmpty(resp)) {
      if (onlyMorning) {
        if (key == this.TODDLER || key == this.PREK || key == this.PRESCHOOL) {
          ...
          resp.morning_nap_start_time = this.convertStartAndEndTime(
            data?.start_time_hour,
            data?.start_time_minute,
            data?.start_time_format
          );
          resp.morning_nap_end_time = this.convertStartAndEndTime(
            data?.end_time_hour,
        ...
        resp.morning_nap_start_time = this.convertStartAndEndTime(
          data?.morning_start_hour,
          data?.morning_start_minute,
          data?.morning_start_format
        );
        resp.morning_nap_end_time = this.convertStartAndEndTime(
          data?.morning_end_hour,
          data?.morning_end_minute,
          data?.morning_end_format
        );
        resp.afternoon_nap_start_time = this.convertStartAndEndTime(
          data?.afternoon_start_hour,
          data?.afternoon_start_minute,
          data?.afternoon_start_format
        );
        resp.afternoon_nap_end_time = this.convertStartAndEndTime(
          data?.afternoon_end_hour,
          data?.afternoon_end_minute,
          data?.afternoon_end_format
        );
      }
  ...
  patchNapData(infantData: any, toddlerData: any) {
    let resp: any = {
      ...
      not_applicable:
        (infantData.morning_nap_start_time != null ||
          toddlerData.morning_nap_start_time != null) &&
        !infantData.is_nap_time &&
        !toddlerData.is_nap_time
          ? true
          : false,
    };
    if (infantData.is_nap_time) {
      resp.morning_start_hour = infantData.morning_nap_start_time
        ? this.providerService.twelveHourFormatFromTwentyFour(
            infantData.morning_nap_start_time,
            'hour'
          )
        : null;
      resp.morning_start_minute = infantData.morning_nap_start_time
        ? this.providerService.twelveHourFormatFromTwentyFour(
            infantData.morning_nap_start_time,
            'minutes'
          )
        : null;
      resp.morning_start_format = infantData.morning_nap_start_time
        ? this.providerService.twelveHourFormatFromTwentyFour(
            infantData.morning_nap_start_time,
            'format'
          )
        : null;
      resp.morning_end_hour = infantData.morning_nap_end_time
        ? this.providerService.twelveHourFormatFromTwentyFour(
            infantData.morning_nap_end_time,
            'hour'
          )
        : null;
      resp.morning_end_minute = infantData.morning_nap_end_time
        ? this.providerService.twelveHourFormatFromTwentyFour(
            infantData.morning_nap_end_time,
            'minutes'
          )
        : null;
      resp.morning_end_format = infantData.morning_nap_end_time
        ? this.providerService.twelveHourFormatFromTwentyFour(
            infantData.morning_nap_end_time,
            'format'
          )
        : null;
      resp.afternoon_start_hour = infantData.afternoon_nap_start_time
        ? this.providerService.twelveHourFormatFromTwentyFour(
            infantData.afternoon_nap_start_time,
            'hour'
          )
        : null;
      resp.afternoon_start_minute = infantData.afternoon_nap_start_time
        ? this.providerService.twelveHourFormatFromTwentyFour(
            infantData.afternoon_nap_start_time,
            'minutes'
          )
        : null;
      resp.afternoon_start_format = infantData.afternoon_nap_start_time
        ? this.providerService.twelveHourFormatFromTwentyFour(
            infantData.afternoon_nap_start_time,
            'format'
          )
        : null;
      resp.afternoon_end_hour = infantData.afternoon_nap_end_time
        ? this.providerService.twelveHourFormatFromTwentyFour(
            infantData.afternoon_nap_end_time,
            'hour'
          )
        : null;
      resp.afternoon_end_minute = infantData.afternoon_nap_end_time
        ? this.providerService.twelveHourFormatFromTwentyFour(
            infantData.afternoon_nap_end_time,
            'minutes'
          )
        : null;
      resp.afternoon_end_format = infantData.afternoon_nap_end_time
        ? this.providerService.twelveHourFormatFromTwentyFour(
            infantData.afternoon_nap_end_time,
            'format'
          )
    ...
    if (toddlerData.is_nap_time) {
      resp.start_time_hour = toddlerData.morning_nap_start_time
        ? this.providerService.twelveHourFormatFromTwentyFour(
            toddlerData.morning_nap_start_time,
            'hour'
          )
        : null;
      resp.start_time_minute = toddlerData.morning_nap_start_time
        ? this.providerService.twelveHourFormatFromTwentyFour(
            toddlerData.morning_nap_start_time,
            'minutes'
          )
        : null;
      resp.start_time_format = toddlerData.morning_nap_start_time
        ? this.providerService.twelveHourFormatFromTwentyFour(
            toddlerData.morning_nap_start_time,
            'format'
          )
        : null;
      resp.end_time_hour = toddlerData.morning_nap_end_time
        ? this.providerService.twelveHourFormatFromTwentyFour(
            toddlerData.morning_nap_end_time,
            'hour'
          )
        : null;
      resp.end_time_minute = toddlerData.morning_nap_end_time
        ? this.providerService.twelveHourFormatFromTwentyFour(
            toddlerData.morning_nap_end_time,
            'minutes'
          )
        : null;
      resp.end_time_format = toddlerData.morning_nap_end_time
        ? this.providerService.twelveHourFormatFromTwentyFour(
            toddlerData.morning_nap_end_time,
            'format'
          )
