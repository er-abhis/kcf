...
@Component({
  selector: 'app-daycare-basic-info',
  templateUrl: './daycare-basic-info.component.html',
  styleUrls: ['./daycare-basic-info.component.scss'],
})
export class DaycareBasicInfoComponent implements OnInit {
          ...
          next: (data: any) => {
            ...
            if (resData.status == false) {
              ...
              this.notificationService.showSuccess(
                'A One time Passcode (OTP) has been sent to ' +
                  dataToSend.email +
                  '. Please check your email and enter in the OTP to login to your account.'
              );
              this.openModal();
            }
          },
                ...
                this.providerService
                  ...
                  .subscribe((data: any) => {
                    if (data && data?.data && data?.code == 200) {
                      ...
                      }/preschool/${data?.data[0]?.state?.replaceAll(
                        ' ',
                        ''
                      )}/${data?.data[0]?.city?.replaceAll(
                        ' ',
                        ''
                      )}/${this.preSchoolGroup.value.organization_name?.replaceAll(
                        ' ',
                        ''
                      )}/${this.providerId}`;
                      this.providerService
                        .updateProviderSignupSteps(
                          { affiliated_provider_url: providerUrl },
                          this.providerId
                        )
                        .subscribe((res) => {
