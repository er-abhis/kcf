...
@Component({
  selector: 'app-meals-snacks',
  templateUrl: './meals-snacks.component.html',
  styleUrls: ['./meals-snacks.component.scss']
})
export class MealsSnacksComponent implements OnInit {
  ...
  checkFormValidity(stepCount: number, callback: any) {
    switch (stepCount) {
      case 0:
        if(this.childNeedCareGroup.value.is_meal_provided == 'Yes'
        && !this.isBreakFast
        && !this.isLunch
        && !this.isDinner){
          this.notificationService.showWarning(
            'Please Select provider meals type'
          );
          ...
          .subscribe({
            next: (data: any) => {
              ...
              if (this.childNeedCareGroup.invalid) {
                ...
                return;
              } else if (this.childNeedCareGroup.valid) {
      ...
      case 1:
        if(this.ChildSnacksFormGroup.value.provideSnacks == 'Yes'
        && !this.isMorning
        && !this.isEvening
        && !this.isAfternoon){
          this.notificationService.showWarning(
